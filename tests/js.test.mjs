...
test('Linear CLI rejects invalid priorities before credentials or waiting for stdin',async()=>{
 ...
 try{
  ...
  for(const file of ['linear_waits.mjs','linear_landing.mjs','linear_skills.mjs','linear_agent.mjs','linear_watch.mjs','linear_workers.mjs','linear_telemetry.mjs', 'linear_wakes.mjs','linear_activity.mjs'])
...
test('create rejects missing or conflicting expectations before loading credentials',async t=>{
 ...
 for(const name of ['linear_waits.mjs','linear_landing.mjs','linear_skills.mjs','linear_agent.mjs','linear_watch.mjs','linear_workers.mjs','linear_telemetry.mjs', 'linear_wakes.mjs','linear_activity.mjs'])await fs.copyFile(new URL('../scripts/'+name,import.meta.url),path.join(dir,'scripts',name));
 ...
 for(const [command,schema] of Object.entries(COMMAND_FLAGS)){
  for(const [action,options] of Array.isArray(schema)?[[null,schema]]:Object.entries(schema)){
   ...
   for(const option of options){
    const toggle=option==='peek'||option==='if-stale';
...
async function flagCliFixture(t){
 ...
 for(const name of ['linear_waits.mjs','linear_landing.mjs','linear_skills.mjs','linear_agent.mjs','linear_watch.mjs','linear_workers.mjs','linear_telemetry.mjs', 'linear_wakes.mjs','linear_activity.mjs'])
