...
test('Linear CLI rejects invalid priorities before credentials or waiting for stdin',async()=>{
 ...
 try{
  ...
  for(const file of ['linear_validation.mjs', 'linear_waits.mjs','linear_landing.mjs','linear_skills.mjs','linear_agent.mjs','linear_watch.mjs','linear_workers.mjs','linear_telemetry.mjs', 'linear_wakes.mjs', 'linear_attribution.mjs','linear_activity.mjs','linear_answers.mjs'])
...
test('only the phases that write contend for files',()=>{
 const claims=[
  ...
  {issue:'JAU-119',phase:'awaiting-validation'},
 ];
 assert.deepEqual(contendingClaims(claims).map((c)=>c.issue),['JAU-45','JAU-14','JAU-119']);
 assert.deepEqual(CONTENDING_PHASES,['implementing','landing','awaiting-validation']);
...
test('create rejects missing or conflicting expectations before loading credentials',async t=>{
 ...
 for(const name of ['linear_validation.mjs', 'linear_waits.mjs','linear_landing.mjs','linear_skills.mjs','linear_agent.mjs','linear_watch.mjs','linear_workers.mjs','linear_telemetry.mjs', 'linear_wakes.mjs', 'linear_attribution.mjs','linear_activity.mjs','linear_answers.mjs'])await fs.copyFile(new URL('../scripts/'+name,import.meta.url),path.join(dir,'scripts',name));
...
async function flagCliFixture(t){
 ...
 for(const name of ['linear_validation.mjs', 'linear_waits.mjs','linear_landing.mjs','linear_skills.mjs','linear_agent.mjs','linear_watch.mjs','linear_workers.mjs','linear_telemetry.mjs', 'linear_wakes.mjs', 'linear_attribution.mjs','linear_activity.mjs','linear_answers.mjs'])
