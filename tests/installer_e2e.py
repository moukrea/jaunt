...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='jaunt-install-test-') as tmp:
        ...
        config={'version':1,'relay':f'ws://127.0.0.1:{relayport}','release':'v0.1.0-beta.4','page':url+'/'}
        ...
        try:
            ...
            assert cli('--version').strip()=='0.1.0b4';passed('wheel installed in private runtime; actual daemon connects to relay')
