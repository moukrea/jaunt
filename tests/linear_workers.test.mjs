...
for (const runtime of ['codex', 'claude']) test(`${runtime}: offline crash, durable watchdog wake and one exact-session recovery`, { timeout: 20000 }, async () => temporary(async dir => {
  ...
  for (const name of ['linear_skills.mjs', 'linear_codex.mjs', 'linear_workers.mjs', 'linear_claude.mjs']) await copyFile(new URL('../scripts/' + name, import.meta.url), join(dir, 'scripts', name));
