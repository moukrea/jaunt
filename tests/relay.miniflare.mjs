...
async function upgrade(mf){const response=await mf.dispatchFetch('https://relay.test/v1/room/'+'r'.repeat(24),{headers:{Upgrade:'websocket',Origin:'https://moukrea.github.io'}});assert.equal(response.status,101);const ws=response.webSocket;assert.ok(ws);ws.accept();return ws;}
test('workerd upgrade, Durable Object enrollment, relay routing and hibernation ping',async()=>{
 ...
 try{
  ...
  client=await upgrade(mf);ready=next(client);const joined=next(host);client.send(JSON.stringify({type:'auth',role:'client',token:'c'.repeat(43)}));assert.equal((await ready).hostOnline,true);const peer=(await joined).peer;
