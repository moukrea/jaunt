 ...
 try:
  async with async_playwright() as pw:
   ...
   for width,name in [(1300,'desktop'),(390,'mobile')]:
    ...
    await sim(1700);await expect(p.locator('#topbar')).to_have_class(re.compile("high-latency"))   # hysteresis: still high under 2 s
    await sim(1200);await expect(p.locator('#topbar')).not_to_have_class(re.compile("high-latency"))
    print(f'PASS {name}: ≥2 s recolours the top bar with the bold warning and the figure; hysteresis avoids flicker')
    await sim(20000);await expect(p.locator('#latency-overlay')).to_be_visible();await expect(p.locator('#latency-overlay')).to_contain_text('20 seconds');await expect(p.locator('#topbar')).to_have_class(re.compile("high-latency"))
    ...
    await sim(900);await expect(p.locator('#latency-overlay')).to_be_hidden();await expect(p.locator('#topbar')).not_to_have_class(re.compile("high-latency"))
