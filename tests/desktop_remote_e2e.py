...
async def main():
    ...
    try:
        ...
        async with async_playwright() as pw:
            ...
            # App settings: this device and the machines it remembers. Nothing a host decides.
            ...
            remote=page.locator('.machine-row').filter(has_text=h.name)
            await remote.get_by_label('Friendly host name').fill('Remote fixture')
            await remote.get_by_label('Friendly host name').press('Tab')
            await expect(page.locator('#machine-title')).to_have_text('Remote fixture')
            await expect(page.locator('.machine-row').filter(has_text='Remote fixture')).to_have_count(1)
            # Host settings follow the selected machine: the local computer offers its service, the remote one does not.
