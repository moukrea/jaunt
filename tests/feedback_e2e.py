...
async def main():
 ...
 try:
  async with async_playwright() as pw:
   ...
   await expect(page.locator('.machine-row').filter(has_text='machine X').get_by_label('Friendly host name')).to_have_value('machine X')
   await page.locator('#machine-list').get_by_role('button',name='jaunt workstation').click()
   await expect(page.locator('.machine-row').filter(has_text='jaunt workstation').get_by_label('Friendly host name')).to_have_value('jaunt workstation')
   await page.locator('.machine-row').filter(has_text='jaunt workstation').get_by_label('Friendly host name').fill('machine Y')
   await page.locator('.machine-row').filter(has_text='jaunt workstation').get_by_label('Friendly host name').press('Tab')
   ...
   await page.locator('#settings-button').click();await page.locator('.machine-row').filter(has_text='machine Y').get_by_role('button',name='Choose…',exact=True).click()
   ...
   await page.locator('.machine-row').filter(has_text='machine Y').get_by_role('button',name='Default',exact=True).click();await expect(page.locator('#host-symbol svg')).to_have_attribute('data-icon-name','cloud')
   ...
   await page.locator('#settings-button').click()
   await expect(page.locator('.machine-row').filter(has_text='machine Y').get_by_label('Friendly host name')).to_have_value('machine Y')
