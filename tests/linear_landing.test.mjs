...
test('canonical CLI acquire persists landing and status without touching another claim', async t => {
  ...
  for (const name of ['linear_waits.mjs', 'linear_activity.mjs', 'linear_agent.mjs', 'linear_landing.mjs', 'linear_workers.mjs', 'linear_telemetry.mjs', 'linear_watch.mjs', 'linear_skills.mjs']) await copyFile(new URL(`../scripts/${name}`, import.meta.url), join(f.root, 'scripts', name));
