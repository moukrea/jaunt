...
5. Configure APP_ORIGIN, verify health AND real WebSockets, then set jaunt_RELAY_URL in GitHub. Follow the exclusive automatic publisher activation and recovery procedure in docs/DEPLOYMENT.md. Do not create component tags or dispatch Pages independently: auto-release.yml verifies assets, promotes the three tag variables, deploys the exact source and verifies the public configuration. Report its actual delivery receipt, not merely a merged PR.
