...
- `web/vendor/xterm.mjs` and `xterm.css`: @xterm/xterm 6.0.0, @xterm/addon-fit 0.11.0 and @xterm/addon-webgl 0.19.0, MIT. Built from the exact package-lock.json dependencies by `npm run prepare-web`; the three licenses are preserved in `web/vendor/LICENSE-xterm.txt`.
- `web/vendor/fonts/` and `jetbrains-mono.css`: JetBrains Mono (latin and latin-ext, weights 400 and 700) from @fontsource/jetbrains-mono 5.2.8, SIL Open Font License 1.1. Copied by `npm run prepare-web`; the license is preserved in `web/vendor/LICENSE-jetbrains-mono.txt`.
