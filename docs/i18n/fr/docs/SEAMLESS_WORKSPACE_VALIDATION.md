...
Les versions publiques sont maintenant vérifiées : hôte `0.1.0b11`, bureau `0.1.0-beta.10`, Android `0.1.0-beta.8`. La page publique, les sommes de contrôle, l’installation officielle et le service utilisateur ont été contrôlés dans une VM Ubuntu isolée. La connexion WebSocket réelle, les commandes shell, les transferts comparés octet par octet, la reconnexion et la révocation ont réussi. Le client seul a été installé dans un compte neuf sans CLI ni service hôte. La mise à jour native du bureau 9 vers 10 a conservé les PID de l’hôte et des shells ; une commande a fonctionné avant et après. Dans un émulateur, Android 7 a été mis à jour vers 8 et a retrouvé le même appairage. Les vérifications répétées se terminent avec un seul dialogue indiquant que la version est à jour.
...
Le remplacement compatible du runtime de l’hôte 11 a conservé les processus et l’identité. Il s’agissait d’une réinstallation de la même version. L’ancien hôte 10 exige toujours une première migration protégée : ses shells actifs ne peuvent pas être préservés rétroactivement. Aucun shell personnel n’a été arrêté. Aucun téléphone physique n’a été utilisé : caméra, galerie, notifications sur écran verrouillé et passage Wi-Fi/mobile restent à valider sur appareil réel. Le protocole n’a pas reçu d’audit de sécurité indépendant.
