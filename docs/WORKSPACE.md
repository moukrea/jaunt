...
- **Share open sessions** (Settings → selected machine) makes every client and the host itself show the same tabs, panes, order and active session for that host. The host stores that workspace; opening, closing, reordering or splitting anywhere is followed everywhere, and sessions that end are pruned. Updates carry the revision they were based on; a stale update is refused and the client adopts the host's current workspace instead of overwriting it. The sub-option **Only displayed sessions exist** turns the × into a termination (no choice shown) and hides the Sessions list for that host, because a session that is not displayed no longer exists.
