...
- **Levels**: *ask* (default for an unknown requester; every request waits for an answer on your devices; after 2 minutes it **expires** — the caller is refused, but the journal and the message say *expired*, never *denied*), *trust* for 1 h, 24 h or always, *block*. An expired trust falls back to ask. Revoking a requester removes its row; it asks again at its next request.
...
| Tool | Does | Bounds |
|---|---|---|
| `jaunt_sessions(host?)` | The jaunt shells of a machine (this one when `host` is omitted): name, current directory, program, whether the requester may type there (`ask`, `trust`, `block`). A shell running a Claude Code or Codex session is flagged with the identity that session uses in its own runtime (from `~/.claude/sessions/<pid>.json`, or the bridge thread for Codex) and a note sending the caller to the right channel. | Read-only. |
| `jaunt_type(host?, session, input, enter?)` | Sends the text to that terminal as keystrokes, Enter appended unless `enter: false`. For what a keyboard does; against a shell running an AI session the reply warns that a long or multi-line text lands as a pasted block Enter does not submit. | 16 KiB per call; journaled with the text. |
