...
The selected-host Settings regression is also covered: switching the sidebar while Settings is open now rebuilds host-bound controls immediately, refreshes the selected host's update status, and ignores stale responses from the previous selection. Desktop local service controls only appear for the local host; desktop application updates and notifications remain device settings. Duplicate local host update/restart controls were removed.

Observed validation: `python tests/feedback_e2e.py` paired two isolated real hosts, switched between them without leaving Settings, renamed Y, forgot X and verified Y's shell PID was unchanged. `DISPLAY=:179 python tests/desktop_remote_e2e.py` used Electron's real local connection and the owner's public WSS relay, verified local-service controls disappear on remote selection and return on local selection, then executed and terminated a fixture shell. `python scripts/check_project.py` passed. Initial new-test failures were a hidden mobile-only Settings selector and an assertion racing asynchronous vault persistence; the tests now use the visible desktop Settings button and wait for the saved name before switching. No application assertions were removed.
