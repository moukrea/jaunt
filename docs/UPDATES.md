...
A channel lets a person follow the binaries of one pull request under review instead of production. This section is the contract every surface and the channel publisher implement; [`tests/fixtures/update_channels.json`](../tests/fixtures/update_channels.json) is its executable form and `scripts/update_channels.py` its reference implementation. The host and desktop implement it (below). Released Android clients do not yet: their version parser refuses every candidate tag, and tests assert that until it adopts the contract.
