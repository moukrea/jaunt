...
**Channel list.** `<page>/ch/index.json` lists the published channels: `{"version": 1, "page", "repository", "channels": [{"name", "pr", "title", "n", "release", "desktopRelease", "androidRelease", "releaseSource"}]}`, sorted by name. `title` is the pull request title as display text and may be absent. The list only offers names: a client still fetches and validates `ch/<name>/config.json` before any switch. Channel pages are deployed with production; see [Channel pages](DEPLOYMENT.md#channel-pages).
...
**Switching everything.** Settings → Update channel → **Switch everything** opens one menu for every connected element: this desktop or Android app, the desktop's local host and each directly connected host whose updates the public installer manages (offline and unmanaged hosts are listed as skipped). Its list is `ch/index.json`, validated by `web/js/channels.mjs` (official `page` and `repository`, publishable names only, `main` first), plus **Other…** for a name typed by hand. A browser reads the list from its own Page (`config.json` `page`); the desktop CSP and the Android WebView forbid that fetch, so the desktop's `channels` update action and Android's `app.channels` bridge action read it natively from the official Page. Hosts switch first, each through `updates.configure {channel}`; the app switches itself only once every chosen host has accepted, because the desktop closes to install. A browser never switches itself: moving the web app to `ch/<name>/` belongs to the PWA channel selector. Hosts reached only through another host's agent links are not targets, since linked hosts may not change updates.
