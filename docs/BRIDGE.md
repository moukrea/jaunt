...
- Scope is one host. Sessions on other machines are reached through *Messages between sessions across machines* (Settings → Agents and machines), a separate switch documented in the [agents and machines guide](AGENTS.md); there, unlike here, sessions of the same runtime are relayed too, since neither runtime reaches another machine by itself.
...
- Codex runs a hook from `hooks.json` only once it has been reviewed and trusted; an untrusted hook is skipped silently, with no prompt in some paths (a session resumed with `codex resume`, `--yolo`, headless runs). Turning the switch on is that review: jaunt records the trust of its own hook in `~/.codex/config.toml` (`[hooks.state."<hooks.json>:<event>:<group>:0"] trusted_hash`, the same sha256 of the hook's normalized identity Codex computes, verified against hashes Codex 0.154.0 wrote itself) and removes it when the switch goes off. Nothing else in that file is touched, and a `config.toml` jaunt cannot parse is left alone (the switch then reports that Codex will ask you in its TUI). The file `~/.local/share/jaunt/bridge/last-hook-codex` is touched at every hook run, so "installed but never triggered" is visible (`bridge.status` → `hooksSeen.codex.lastHook`).
