...
jaunt-linear validation review <ID> --pr <n> --snapshot <snapshot> \
  --reason "<how the current instructions are satisfied>" --runtime claude --session "$CLAUDE_CODE_SESSION_ID"
```
...
jaunt-linear validation begin <ID> --reviewer <Linear-human-id> \
  ...
  --runtime claude --session "$CLAUDE_CODE_SESSION_ID"
```
...
`validation decision <ID> --runtime claude --session "$CLAUDE_CODE_SESSION_ID"` records an exact
