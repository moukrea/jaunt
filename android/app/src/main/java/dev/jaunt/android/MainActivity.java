    ...
    private void dispatch(String method,JSONObject p,Reply reply){
        try{
            switch(method){
                ...
                case "qr.scan":if(scanReply!=null)throw new IllegalStateException("Scanner already open");scanReply=reply;new IntentIntegrator(this).setDesiredBarcodeFormats(IntentIntegrator.QR_CODE).setPrompt("Scan the QR from jaunt pair").setBeepEnabled(false).setOrientationLocked(false).initiateScan();return;
